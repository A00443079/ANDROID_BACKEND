...
public class PostDB 
{
    ...
    public static boolean insertNewReservation(String id, ReservationDetails rd)
    {
        ...
        query= "insert into userreservation values(\"" + email + "\", \"" + id + "\")";
        result = template.update(query);	//insert into 3rd table 'userreservation'
        if(result==0) return(false);	//if above insertion fails, then rest of the operations are aborted, overall
