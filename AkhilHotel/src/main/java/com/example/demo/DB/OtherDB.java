...
public class OtherDB 
{
    ...
    public static boolean delete(String id)
    {
        ...
        String query = "delete from userreservation where confirmation_number=\""+id+"\"";
        int result = template.update(query);	
        if(result==0) return(false);	
        
        query = "delete from guest where confirmation_number=\""+id+"\"";
        result = template.update(query);	
        if(result==0) return(false);
        
        query = "delete from reservation where confirmation_number=\""+id+"\"";
        result = template.update(query);	
        if(result==0) return(false);
    ...
    public static ReservationDetails getRD(String id)
    {
        ...
        //STEP 3
        query="select distinct username from userreservation where confirmation_number="+"\""+id+"\"";
        rows = template.queryForList(query);
        ...
        if (rows!=null && !rows.isEmpty()) 
        {
            for (Map<String, Object> row : rows) 
            {
                for (Iterator<Map.Entry<String, Object>> it = row.entrySet().iterator(); it.hasNext();) 
                {
                    ...
                    if(key.equals("username")) rd.setEmail(""+value);
